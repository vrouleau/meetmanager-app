...
| `meet_fees_json` | JSON: `{"CLUB": cents, "ATHLETE": cents, ...}` — parsed from `<MEET><FEES>` in the .lxf |
...
**Meet upload**: Replaces all event data and registrations. Club PINs are regenerated. All metadata goes into `app_config`. **Do not add ALTER TABLE / migration logic** — just update the models directly, SQLAlchemy creates the table fresh on first run.
...
- Per-entry fees: from `Event.fee_cents`. Individual events bill per athlete; relay events bill once per team.
- Items with qty ≤ 0 or cents = 0 are skipped.
- Late fees (LATEFEE) exist in the data but are NOT conditionally gated — the user decided not to implement conditional late-fee logic.
...
**Fee summary**: `FeeSummary` component in `Admin.jsx` — scrollable monospace box showing meet-level fees + per-event fees after the meet info banner.

**SPLASH Quantum**: Custom styles are stored per-session. To seed new sessions, edit `quantum/LSTSTYLE.en-UK` — this is the default template Quantum copies on session creation.
